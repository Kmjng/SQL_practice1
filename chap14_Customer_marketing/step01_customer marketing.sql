...
SELECT customer_id, cname, total_price,
    ...
    END 구매카테고리
FROM customers   
WHERE total_price IS NOT NULL --구매고객 없는 고객 제외
...
SELECT cname, email, address, total_price,
    CASE
    WHEN total_price >=1000 THEN '고가'
    WHEN total_price BETWEEN 500 AND 1000 THEN '중가'
    WHEN total_price < 500 THEN '저가'
    END 구매카테고리 
FROM customers 
WHERE (address  LIKE '%Town%'  OR address LIKE '%Village%' ) AND total_price IS NOT NULL -- ★★★ 다중 LIKE 
ORDER BY total_price DESC; 
...
SELECT cname, birthdate, address, total_price,
    CASE
    WHEN TO_NUMBER(SUBSTR(birthdate, 4, 2)) BETWEEN 03 AND 05 THEN '봄'
    WHEN TO_NUMBER(SUBSTR(birthdate, 4, 2)) BETWEEN 06 AND 08 THEN '여름'
    WHEN TO_NUMBER(SUBSTR(birthdate, 4, 2)) BETWEEN 09 AND 11 THEN '가을'
    WHEN TO_NUMBER(SUBSTR(birthdate, 4, 2)) IN (12,01,02) THEN '겨울'
    END 계절카테고리
FROM customers
WHERE  TO_NUMBER(SUBSTR(birthdate, 4, 2))  IN (3,4,5,6,7,8)
;
...
SELECT cname, birthdate, address, total_price,
    ...
    WHEN SUBSTR(birthdate, 4, 2) >= '12'  AND SUBSTR(birthdate, 4, 2) <= '02'   THEN '겨울'
    END 계절카테고리
FROM customers
...
FROM customers
WHERE churn_status = '탈퇴';


-- 2) 이탈가능고객 조회 : 1개월 이상 구매이력 없는 고객 
-- 개월수 반환 함수 사용 - MONTHS_BETWEEN(날짜1, 날짜2) 
SELECT customer_id, cname, email, total_price, last_date,
    CASE
        WHEN MONTHS_BETWEEN(SYSDATE, last_date) >= 1 THEN '이탈가능' -- last_date: 마지막 구매날짜
        ELSE '지속가능'
    END "이탈여부"
FROM customers
ORDER BY "이탈여부"  ; 
...
SELECT cname, reg_date, phone_number, last_date 
FROM customers
WHERE MONTHS_BETWEEN(SYSDATE, reg_date) < 24 AND sysdate - last_date >= 30 
AND churn_status != '탈퇴' ;
