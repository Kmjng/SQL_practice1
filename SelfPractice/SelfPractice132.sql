...
SELECT c_loc, cnt, RANK() OVER ( ORDER BY cnt DESC ) RANK FROM crime_loc 
WHERE crime_type ='살인' AND RANK = 1; -- [오류] 알고리즘 순서가 SELECT보다 WHERE가 먼저임!!

SELECT c_loc, cnt, RANK FROM (
SELECT c_loc, cnt, RANK() OVER ( ORDER BY cnt DESC ) RANK FROM crime_loc WHERE crime_type = '살인') 
WHERE RANK = 1;
