...
SELECT 범죄유형, 가정불화 FROM crime_cause WHERE 가정불화 = (SELECT MAX(가정불화) FROM crime_cause );

--방법 2. UNPIVOT 사용해서 출력을 용이하게 함. 
CREATE TABLE crime_cause2 
AS SELECT * FROM crime_cause 
UNPIVOT (cnt FOR term IN (생계형, 유흥, 도박, 허영심, 복수, 해고, 징벌, 가정불화, 호기심, 유혹, 사고, 불만, 부주의, 기타 ));
 -- UNPIVOT : 칼럼을 레이블 값으로 치환한다. 
 -- '생계형'...이라는 칼럼들을 레코드 값으로 치환하고, 그 칼럼을 term이라 하겠다. 기존의 레이블 값에 대한 칼럼을 cnt으로 설정하고 출력한다.  
SELECT * FROM crime_cause2;

SELECT 범죄유형 FROM crime_cause2 WHERE cnt = (SELECT MAX(cnt) FROM crime_cause2 WHERE term = '가정불화');