...
SELECT ename, sal
FROM emp
WHERE sal >=1500 AND (deptno = 10 OR deptno =30);  -- deptno IN (10,30) 도 가능
...
-- ★★★ [문8] ename에 L이 두 자가 있고, deptno가 30이거나 또는 mgr이 7902인 모든 사원 출력하기(힌트 : like)
SELECT * 
FROM emp 
WHERE (ename LIKE '%L%L%' AND deptno = 30) OR mgr = 7902 ; 
